...
# Read the data
data <- read_excel("第六次.xlsx", col_names = TRUE)
...
migration_matrix[is.na(migration_matrix)] <- 0

write.xlsx(migration_matrix,"migration_matrix_6.xlsx")
# describe the matrix
...
filtered_matrix_log[filtered_matrix_log < 7] <- 0
...
filtered_matrix_log[filtered_matrix_log < 6.98] <- 0
...
degree_tot <- rowSums(W_normalized > 0) + colSums(W_normalized)
...
result <- tabu_search(g_wei, max_iterations = 100, tabu_list_size = 10, max_no_improve = 40)
cat("Best Modularity:", result$best_modularity, "\n")
print("Best Community Structure:")
print(result$best_community)

community_assignment <- data.frame(Node = V(g_wei)$name, Community = result$best_community)
